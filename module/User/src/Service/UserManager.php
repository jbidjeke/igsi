...
class UserManager
{
    ...
    public function addUser($data) 
    {
        ...
        /*$user->setEmail($data['first_name']);
        $user->setFullName($data['organization']); */       
    ...
    public function createAdminUserIfNotExists()
    {
        ...
        if ($user==null) {
            ...
            $adminRole = $this->entityManager->getRepository(Role::class)
                    ->findOneByName('Super-administrateur');
            if ($adminRole==null) {
                throw new \Exception('Super-administrateur rôle doesn\'t exist');
            }
    ...
    /**
     * This method checks if on user presents, and if not, creates
     * user
     */
    public function createOtherUserIfNotExists($fullName, $email = null, $password = null, $role = null, $status=User::STATUS_DRAFT)
    {
        $user = $this->checkUserExistsByFullName($fullName);
        if ($user==null) {
            ...
            // Assign user draft role
            $result = $this->entityManager->getRepository(Role::class)
            ->findOneByName($role);
            if ($result==null) {
                throw new \Exception($role.' role doesn\'t exist');
            }
    
            $user->getRoles()->add($result);
    ...
    /**
     * Get user with given FullName already exists in the database
     * with status
     */
    public function getUserExistsByFullNameWithStatus($fullName, $status = User::STATUS_ACTIVE) {
    
        $user = $this->entityManager->getRepository(User::class)
        ->findOneBy(['fullName'=>$fullName, 'status' => $status]);
    
        return $user;
    }
    
    /**
     * Checks whether an active user with given FullName already exists in the database
     * with status is draft
     */
    public function checkUserExistsByFullNameWithStatus($fullName, $status = 0) {
     ...
     * Checks whether an active user with given FullName already exists in the database.
     */
    public function checkUserExistsByFullName($fullName) {
    
        $user = $this->entityManager->getRepository(User::class)
        ->findOneByFull_name($fullName);
